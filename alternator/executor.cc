    ...
    if (needs_read_before_write) {
        ...
        if (_write_isolation == write_isolation::UNSAFE_RMW) {
            ...
            return get_previous_item(proxy, client_state, schema(), _pk, _ck, permit, stats).then(
                ...
                return proxy.mutate(std::vector<mutation>{std::move(*m)}, executor::default_write_consistency_level, executor::default_timeout(executor::default_putitem_nonlwt_timeout), trace_state, std::move(permit), db::allow_per_partition_rate_limit::yes).then([this] () mutable {
